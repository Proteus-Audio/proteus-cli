...
pub fn init_hash_buffer(keys: &Vec<i32>, sample_rate: Option<usize>) -> Arc<Mutex<HashMap<i32, Bounded<Vec<f32>>>>> {
    // // Named object for storing decoded audio samples for each of the tracks in the array.
    let track_buffers: Arc<Mutex<HashMap<i32, Bounded<Vec<f32>>>>> = Arc::new(Mutex::new(HashMap::new()));
...
pub fn buffer_remaining_space(track_buffers: &Arc<Mutex<HashMap<i32, Bounded<Vec<f32>>>>>, track_key: i32) -> usize {
    let track_buffers = track_buffers.lock().unwrap();
    let track_buffer = track_buffers.get(&track_key).unwrap();
